...
#define CCS811_SW_RESET 0xFF   // 4 bytes
  ...
  M::M(i2c_port_t i2c_port, CCS811::ADDRESS slaveaddr, CCS811::MODE mode, gpio_num_t nwake) : i2c_sensor(i2c_port, (uint8_t)slaveaddr), mode(mode), nwake(nwake)
  {
  }
  ...
  esp_err_t M::Initialize(int64_t &waitTillFirstTrigger)
  {
    ...
    ok = i2cwrite(CCS811_MEAS_MODE, 1, meas_mode);
    if (!ok)
    {
      ESP_LOGE(TAG, "Setting measurement mode failed");
      goto abort_begin;
    }
    wake_down();
    return ESP_OK;
  ...
  const char *M::errstat_str(uint16_t errstat)
  {
...
#define LO(u16) ((uint8_t)(((u16) >> 0) & 0xFF))
  ...
  bool M::set_envdata(uint16_t t, uint16_t h)
  {
  ...
  bool M::set_envdata210(uint16_t t, uint16_t h)
  {
  ...
  bool M::set_envdata_Celsius_percRH(float t, float h)
  {
  ...
  bool M::get_baseline(uint16_t *baseline)
  {
    uint8_t buf[2];
    wake_up();
  ...
  bool M::set_baseline(uint16_t baseline)
  {
  ...
  bool M::flash(const uint8_t *image, int size)
  {
    uint8_t sw_reset[] = {0x11, 0xE5, 0x72, 0x8A};
