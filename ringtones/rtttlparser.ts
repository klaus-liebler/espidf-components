export class Song{
constructor(public Name:string, public Tones:Array<Tone>, public TheDefaults:Defaults){}
}

export class Tone{
constructor(public Frequency: number, public Duration: number, public Name:string){}
}
 ...
 */
export function parse(rtttl:string):Song {
  ...
  var def=getDefaults(name, SECTIONS[1])
  var tones = getData(SECTIONS[2], def)
  return new Song(name, tones, def);
}
 ...
 * @returns {Array}
 */
export function getData(melody:string, defaults:Defaults):Array<Tone> {
  ...
  return NOTES.map((note) => {
    ...
    return new Tone(_calculateFrequency(NOTE, NOTE_OCTAVE), _calculateDuration(msPerFullNote, NOTE_DURATION!, NOTE_DOTTED), NOTE)
