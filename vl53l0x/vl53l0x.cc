...
namespace VL53L0X
{
    ...
    ErrorCode M::Readout(int64_t& waitTillNextTrigger){
        waitTillNextTrigger=250;
        this->lastMeasurementMillimeters=readRangeContinuousMillimeters();
        return ErrorCode::OK;
    }
    ErrorCode M::Initialize(int64_t& waitTillFirstTrigger){
        waitTillFirstTrigger=250;
        if(!init(true)){
    ...
    bool M::setSignalRateLimit(float limit_Mcps)
    {
    ...
    bool M::setMeasurementTimingBudget(uint32_t budget_us)
    {
    ...
    // in us
    uint32_t M::getMeasurementTimingBudget()
    {
        SequenceStepEnables enables;
        SequenceStepTimeouts timeouts;
    ...
    bool M::setVcselPulsePeriod(vcselPeriodType type, uint8_t period_pclks)
    {
    ...
    uint8_t M::getVcselPulsePeriod(vcselPeriodType type)
    {
        if (type == VcselPeriodPreRange)
        {
    ...
    void M::startContinuous(uint32_t period_ms)
    {
        writeReg(0x80, 0x01);
        writeReg(0xFF, 0x01);
        writeReg(0x00, 0x00);
    ...
    uint16_t M::readRangeContinuousMillimeters()
    {
        startTimeout();
    ...
    uint16_t M::readRangeSingleMillimeters()
    {
        writeReg(0x80, 0x01);
        writeReg(0xFF, 0x01);
        writeReg(0x00, 0x00);
    ...
    bool M::getSpadInfo(uint8_t *count, bool *type_is_aperture)
    {
        uint8_t tmp;
    ...
    void M::getSequenceStepEnables(SequenceStepEnables *enables)
    {
        uint8_t sequence_config = readReg(SYSTEM_SEQUENCE_CONFIG);
    ...
    // intermediate values
    void M::getSequenceStepTimeouts(SequenceStepEnables const *enables, SequenceStepTimeouts *timeouts)
    {
    ...
    uint16_t M::decodeTimeout(uint16_t reg_val)
    {
    ...
    uint16_t M::encodeTimeout(uint32_t timeout_mclks)
    {
        // format: "(LSByte * 2^MSByte) + 1"
    ...
    uint32_t M::timeoutMclksToMicroseconds(uint16_t timeout_period_mclks, uint8_t vcsel_period_pclks)
    {
    ...
    uint32_t M::timeoutMicrosecondsToMclks(uint32_t timeout_period_us, uint8_t vcsel_period_pclks)
    {
        uint32_t macro_period_ns = calcMacroPeriod(vcsel_period_pclks);
    ...
    bool M::performSingleRefCalibration(uint8_t vhv_init_byte)
    {
