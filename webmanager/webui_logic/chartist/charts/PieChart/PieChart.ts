...
import type { ResponsiveOptions } from '../../core/index';
import {
  alphaNumerate,
  quantity,
  normalizeData,
...
export class PieChart extends BaseChart<PieChartEventsTypes> {
  ...
  createChart(options: PieChartOptionsWithDefaults) {
    ...
    const hasSingleValInSeries =
      data.series.filter((val: any) =>
        safeHasProperty(val, 'value') ? val.value !== 0 : val !== 0
      ).length === 1;

    // Creating the series groups
    data.series.forEach((_: any, index: any) => (seriesGroups[index] = svg.elem('g')));
